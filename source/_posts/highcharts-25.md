...
那上圖就是我們今天要製作圖表，其實分析一下就可以發現金字塔圖表就只是一個「雙Ｘ軸長條圖」，反而它的Ｙ軸座標比較特別，最小刻度竟然在中間，越往兩側值越高，但其實這只是個障眼法，下面就來分享一下背後的技巧吧！
...
再來就是這次「障眼法」的重點了，其實今天的原始資料都已經可以直接拿來使用了，但男性的資料我們要稍微動一下手腳。透過 `map` 把男性資料乘以 `-1`，這樣數據列剛好一組正數一組負數，自然就會往兩邊延伸了。
