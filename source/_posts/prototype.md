---
title: 面試官最愛考的 JS 原型鍊
date: 2020/7/25 16:55:00
tags: [JavaScript,原型鍊]
---
> JavaScript 是一種基於原型，而不是基於類的物件導向語言。由於這個根本的區別，使它在如何創建物件的層級結構，以及如何繼承屬性和它的值上，顯得有點模糊。 -- *MDN*

JavaScript 的原型鍊一直都是大家比較懼怕的一部分，但其實並不是因為它過於艱深，反而是因為它的概念很抽象，且初期的前端開發中也不是常常遇到，導致要理解起來會比較困難。

雖然我的說明可能比較瑣碎，但希望用我的思考方式能夠帶大家認識 JavaScript 的原型鍊。
...
可能不是每個人都能理解它的意思，但沒關係，我們今天不會深入這個主題，會提到物件導向是因為 JavaScript 是一個支援物件導向設計的程式語言，而能夠支援的秘密就藏在原型練裡面。
...
<p style="font-size:12px; line-height:1.5;">透過上面的描述我們知道了 JavaScript 並沒有類別與實體，但你可能聽過有人會在 JavaScript 的領域提到過類別，或是看過 ES6 的 Class，這是怎麼回事呢？這件事情其實常常被誤會，或許是因為基於類別的程式語言比較廣為使用，所以大家就以習慣的「類別」和「實例」來稱呼 JavaScript 的建構函式和物件了，而 ES6 的 Class，僅僅只是簡化原型練操作的語法糖。</p>
...
若你不曾接觸過原型鍊或建構函式，你可能會定義一個 `truck` 物件，然後調整一下屬性的值。
...
參考資料：
- [該來理解 JavaScript 的原型鍊了 - huli](https://blog.techbridge.cc/2017/04/22/javascript-prototype/)
